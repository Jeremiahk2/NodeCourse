...
describe('DataBase test suite', ()=> {
    ...
    it('should find all elements with the same property', async ()=> {
        const id1 = await sut.insert(someObject1);
        const id2 = await sut.insert(someObject2);
    ...
    it('should return all objects', async ()=> {
        const id1 = await sut.insert(someObject1);
        const id2 = await sut.insert(someObject2);
        const id3 = await sut.insert(someObject3);
        const expected = [someObject1, someObject2, someObject3];
